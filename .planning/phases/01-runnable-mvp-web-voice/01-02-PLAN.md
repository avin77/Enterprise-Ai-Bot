...
must_haves:
  ...
  artifacts:
    ...
    - path: "knowledge/pipeline/embed.py"
      provides: "all-MiniLM-L6-v2 embedding + DynamoDB BatchWriteItem storage"
...
<task type="auto" tdd="true">
  ...
  <action>
    ...
    class KnowledgeResult:
        ...
        sources: list[str]       # Source document names for citation
...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - sample_faqs.json has >=10 Jackson County FAQ entries with keys: question, answer, source_doc, department, chunk_id
  ...
  <action>
    ...
    Each entry: {"question": str, "answer": str, "source_doc": str, "department": str, "chunk_id": str}
  ...
  <done>
    All 10 Wave 0 test stubs now pass (no more skips or ImportError). PipelineResult has asr_ms, rag_ms, llm_ms, tts_ms fields. run_roundtrip produces non-zero timing values. test_full_voice_turn_returns_sources confirms sources list is non-empty. Phase 0 tests remain green.
  </done>
</task>
...
- sample_faqs.json has >=10 Jackson County FAQ entries
...
- DynamoDB schema (PK/SK) confirmed
